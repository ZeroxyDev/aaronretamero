...
- The default typeface is always `Site Font` from `/font.woff2` with `font-weight: 100 900`, `font-style: normal`, `font-display: swap`, and `font-variation-settings: "slnt" 0` both in `@font-face` and as the default body variation.
- Generated canvas images must also use `Site Font`; load it through `document.fonts` before drawing text and explicitly apply `"slnt" 0` when the canvas API supports font variation settings.
